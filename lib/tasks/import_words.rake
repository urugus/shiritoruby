...
namespace :words do
  ...
  def extract_words_from_rdoc(base_path)
    ...
    Dir.glob("#{base_path}/**/*.rd").each do |file|
      ...
      content.scan(/\b[A-Z][A-Za-z0-9_]*[a-z][A-Za-z0-9_]*\b/).each do |word|
        # 先頭も末尾もアルファベットである単語のみ追加
        if word.match?(/\A[A-Za-z0-9_]+\z/) && word.match?(/\A[A-Za-z]/) && word.match?(/[A-Za-z]\z/)
          words.add(word)
        end
      end
  ...
  def import_words(words)
    words.each do |word|
      # 単語の先頭がアルファベットでない、または末尾がアルファベットでない場合はスキップ
      if !word.match?(/\A[A-Za-z]/)
        puts "\nSkipped '#{word}': Word does not start with a letter"
        next
      elsif !word.match?(/[A-Za-z]\z/)
        puts "\nSkipped '#{word}': Word does not end with a letter"
        next
      end
